...
public class CreateSourceFileMember extends JDialog {
    ...
    protected void createMember(int currentX, int currentY) {
        ...
        try {
            ...
            exc.printStackTrace();
            row = "Error: Creating source member  " + memberName + " - " + exc.toString() + ".";
            mainWindow.msgVector.add(row);
            mainWindow.showMessages(true);
            return;
        }

        row = "Comp: Source member " + memberName + "  was created in  " + libraryName + "/" + fileName + " source file.";
        mainWindow.msgVector.add(row);
        mainWindow.showMessages(true);
        
        //System.out.println("rightRow: "+mainWindow.rightRow);        
        // Expand parent node - with added member included
        //mainWindow.rightTree.expandRow(mainWindow.rightRow);  // DOES NOT WORK
    }
