...
/*      This was removed because tne method was moved to the extra class "CreateSourceFileMember"!
...
*/
    ...
    protected void createIfsDirectory() {
        ...
        String directoryName = new GetTextFromDialog("CREATE NEW DIRECTORY", "Parent directory", "New directory name", 
                mainWindow.rightPathString + "/", "", false, currentX, currentY).evaluateTextField();
    ...
    protected void createIfsFile() {
        ...
        fileName = new GetTextFromDialog("CREATE NEW FILE", "Parent directory", "New file name", 
                mainWindow.rightPathString + "/", "", false, currentX, currentY).evaluateTextField();
    ...
    /**
     -----------------------
     * Create Source Member.  This method was replaced to the extra class "CreateSourceFileMember"!
     *
     -----------------------
     */
/*        
...
*/
    ...
    protected void createSourcePhysicalFile() {
        ...
        String sourceFileName = new GetTextFromDialog("CREATE NEW SOURCE PHYSICAL FILE", "Library", "Source physical file name", 
                libraryName, "", false, currentX, currentY).evaluateTextField();
    ...
    protected void createSaveFile() {
        ...
        saveFileName = new GetTextFromDialog("CREATE NEW SAVE FILE", "Library", "Save file name", 
                libraryName, "", false, currentX, currentY).evaluateTextField();
    ...
    protected void clearSaveFile() {
        ...
        extractNamesFromIfsPath(mainWindow.rightPathString);
        // First confirm or deny deletion
        boolean answer = new ConfirmDenyDialog("CLEAR SAVE FILE", "IFS object name", "Save file",
                mainWindow.rightPathString, saveFileName, currentX, currentY).returnAnswer();
        if (answer) {  // If dialog confirmed, clear the save file
            ...
            try {
                ...
                // Expand parent node - without the deleted directory
        ...
        } else {  // If dialog denied, keep the save file untouched
            // Reload the file and expand the parent (library)
            mainWindow.rightNode = (DefaultMutableTreeNode) mainWindow.rightNode.getParent();
    ...
    protected void deleteSourceMember() {
        // Source member was recognized as file with path containing ".FILE/"

        extractNamesFromIfsPath(mainWindow.rightPathString);

        // First confirm or deny deletion
        boolean answer = new ConfirmDenyDialog("DELETE SOURCE MEMBER", "IFS object name", "Member",
                mainWindow.rightPathString, memberName, currentX, currentY).returnAnswer();
        if (answer) {  // If dialog confirmed, delete the file (source member)
        ...
        } else {  // If dialog denied, keep the directory (source file) untouched
            // Reload the directory and expand the parent (library)
            mainWindow.rightNode = (DefaultMutableTreeNode) mainWindow.rightNode.getParent();
            mainWindow.rightTreeModel.reload(mainWindow.rightNode);
            mainWindow.rightTree.expandRow(0);
        }
    ...
    protected void deleteSourcePhysicalFile() {
        ...
        // First confirm or deny deletion
        boolean answer = new ConfirmDenyDialog("DELETE SOURCE PHYSICAL FILE", "IFS object name", "File name",
                ifsFile.toString(), fileName, currentX, currentY).returnAnswer();
        if (answer) {  // If dialog confirmed, delete the directory (source physical file)
            ...
            try {
                ...
                if (ifsFile.isSourcePhysicalFile()) {
                    ...
                    // Expand parent node - without the deleted directory  (source file)
                    mainWindow.rightNode = (DefaultMutableTreeNode) mainWindow.rightNode.getParent();
                    mainWindow.rightTree.expandRow(0);
                }
    ...
    protected void deleteSaveFile() {
        ...
        // First confirm or deny deletion
        boolean answer = new ConfirmDenyDialog("DELETE SAVE FILE", "IFS object name", "Save file name",
                mainWindow.rightPathString, saveFileName, currentX, currentY).returnAnswer();
        if (answer) {  // If dialog confirmed, delete the directory (source physical file)
    ...
    protected void deleteIfsObject() {
        ...
        try {
            if (!ifsFile.isDirectory()) {
                ...
                // First confirm or deny deletion
                String filename = ifsFile.toString().substring(ifsFile.toString().lastIndexOf("/") + 1);
                boolean answer = new ConfirmDenyDialog("DELETE IFS FILE", "IFS object path", "File name",
                        ifsFile.toString(), filename, currentX, currentY).returnAnswer();
                if (answer) {
                    ...
                    // Reload parent node - keep the directory untouched
                    mainWindow.rightNode = (DefaultMutableTreeNode) mainWindow.rightNode.getParent();
                    mainWindow.rightTreeModel.reload(mainWindow.rightNode);                    
                }
            } else { //
                ...
                // First confirm or deny deletion
                String dirname = ifsFile.toString().substring(ifsFile.toString().lastIndexOf("/") + 1);
                boolean answer = new ConfirmDenyDialog("DELETE IFS DIRECTORY", "IFS object path", "Directory name",
                        ifsFile.toString(), dirname, currentX, currentY).returnAnswer();
                if (answer) {
    ...
    protected void copyLibrary() {
        ...
        String newLibraryName = new GetTextFromDialog("COPY LIBRARY", "Library name", "New library name", 
                libraryName, "", false, currentX, currentY).evaluateTextField();
        // Enable calling CL commands
        CommandCall command = new CommandCall(remoteServer);
        ...
        // PARENT NODE of created node will be reloaded.
        // Get parent path string
    ...
    protected void clearLibrary() {
        ...
        // First confirm or deny deletion
        //String dirname = mainWindow.rightPathString.substring(ifsFile.toString().lastIndexOf("/") + 1);
        boolean answer = new ConfirmDenyDialog("CLEAR LIBRARY", "IFS object path", "Library name",
                ifsFile.toString(), libraryName, currentX, currentY).returnAnswer();
        if (answer) {
        ...
        } else {  // If dialog denied, keep the library
            // Reload parent node - keep the directory untouched
            mainWindow.rightNode = (DefaultMutableTreeNode) mainWindow.rightNode.getParent();
            mainWindow.rightTreeModel.reload(mainWindow.rightNode);
        }
    ...
    protected void deleteLibrary() {
        ...
        // First confirm or deny deletion
        boolean answer = new ConfirmDenyDialog("DELETE LIBRARY", "IFS object name", "Library name",
                mainWindow.rightPathString, libraryName, currentX, currentY).returnAnswer();
        if (answer) {  // If dialog confirmed, delete the library
            // Delete the library
            // ------------------
            try {
                // Run the command
